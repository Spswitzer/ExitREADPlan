...
studentStrings <- data.frame(planStatus = c('planEnd', 'NoPlanEnd', 
                                            'exitCmasMeetExceed', 'noExitCmas'), 
                             personIds = c(toString(planEnd), toString(NoPlanEnd), 
                                           toString(exitCmasMeetExceed), toString(noExitCmas)))
...
noInstYesFlag <- instructionAndFlag %>% 
  full_join(planFlag, join_by(personID == personID)) %>% 
  filter(is.na(plan))
# 49 students with no documented instructional plan
